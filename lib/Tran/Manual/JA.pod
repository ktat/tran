...
今まで、PerlのモジュールやPerlのコアドキュメントをいくつか訳してきましたが、最近は、あんまり訳していません。
...
今のところ、cmpmerge と cmpmerge_least の2つしかありません。
...
のように指定することで、マージされた結果が標準出力に表示されます。
新しい翻訳ファイルを最後の引数に渡した場合は、そこに書き込まれます。
